...
class TaskModelTest(TestCase):
    def setUp(self):
        ...
        self.task = Task.objects.create(
            user=self.user, title="Test task", description="Test description"
        )
...
class TaskViewTest(TestCase):
    def setUp(self):
        ...
        self.task = Task.objects.create(
            user=self.user, title="Test task", description="Test content"
        )
    ...
    def test_task_create_view(self):
        response = self.client.post(
            reverse("task-create"),
            {"title": "New task", "description": "New description", "complete": False},
        )
        self.assertEqual(response.status_code, 302)
...
class TaskPermissionTest(TestCase):
    def setUp(self):
        ...
        self.task = Task.objects.create(
            user=self.user1, title="User1 Task", description="Test content"
        )
...
class TaskReorderTest(TestCase):
    def setUp(self):
        ...
        self.tasks = [
            Task.objects.create(user=self.user, title=f"Task {i}") for i in range(3)
        ]
    ...
    def test_task_reorder(self):
        ...
        response = self.client.post(
            reverse("task-reorder"), {"position": ",".join(new_order)}
        )
        self.assertEqual(response.status_code, 302)
...
class TemplateTest(TestCase):
    def setUp(self):
        ...
        self.incomplete_task = Task.objects.create(
            user=self.user, title="Incomplete Task", complete=False
        )
        self.client.login(username="testuser", password="testpass123")
